...
function buildCsvPreview(buffer) {
  ...
  return {
    ...
    previewRows: rows,
    previewTotalRows: Array.isArray(parsed.data) ? parsed.data.length : 0,
  };
}
...
async function handleUpload(req, res) {
  ...
  const duplicate = await CsvFile.findOne({
  ...
  if (duplicate) {
    return res.status(409).json({
      error: "Duplicate CSV already uploaded",
  ...
  const preview = buildCsvPreview(req.file.buffer);
  ...
  const file = await CsvFile.create({
    ...
    parsedCount: parsed.trades.length,
    skippedCount: parsed.skippedCount,
    filteredByStatus: parsed.filteredByStatus,
    warnings: [...parsed.warnings, ...parsed.metaWarnings],

    previewHeaders: preview.previewHeaders,
    previewRows: preview.previewRows,
    previewTotalRows: preview.previewTotalRows,
  });
